...
const WithdrawBTC = () => {
  ...
  const onChangeBTCAddress = (e) => {
    const btcAddress = e.target.value;
  ...
  const withdrawBTC = (e) => {
    ...
    setSuccessful(false);
    showLoading();
    WithdrawService.withdraw_btc(accNumber, withAmount, btcAddress).then(
      (response) => {
        hideLoading();
              ...
              {!successful && (
                <div>
                  <div className="">
                    ...
                    <label className="text-xs text-gray-600 font-bold mt-3">Withdraw amount in BTC</label>
                    <CurrencyInput
                      id="input-example"
                      name="amount"
                      ...
                      className="mb-1"
                    />
                    <p className="text-gray-600 text-xs">Your balance: {btcBalance.toFixed(3)}</p>
                    {btcBalance < withAmount &&
                      <p className="text-red-600 text-xs">Not enough balance</p>
                    }
                  </div>
                  <button disabled={!(withAmount && btcAddress) || btcBalance < withAmount || !validate(btcAddress)} className="btn-unstake disabled:bg-gray-200 disabled:border-gray-200 disabled:text-gray-400" tabIndex={-1} onClick={() => setShowModal(true)}>
                    <span>Retirar</span>
                  </button>
                </div>
              )}
        ...
        <div className="fixed top-0 left-0 right-0 z-50 w-full p-4 overflow-x-hidden overflow-y-auto bg-black bg-opacity-30 md:inset-0 h-modal md:h-full">
          <div className="flex justify-center mt-20">
            <div className="shadow-md rounded-md text-center bg-white px-10 py-5 text-gray-600">
              <h1 className="mb-5">Do you really want to withdraw <b>{withdrawAmount}</b> BTC?</h1>
              <div className="flex justify-end items-center">
                <button className="text-white bg-black hover:bg-gray-800 focus:ring-4 focus:outline-none focus:ring-gray-500  font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2" onClick={withdrawBTC}>Yes</button>
