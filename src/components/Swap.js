...
const Swap = () => {
  ...
  const onChangeDepositAmount = (e) => {
    ...
    const usdtRate = UserService.get_usdt_rate().then((response) => {
      const deposit = parseInt(
        depositAmount.split(",").join("").split("$").join("")
      );
  ...
  const handleExchange = (e) => {
    ...
    if (checkBtn.current.context._errors.length === 0) {
      UserService.exchange_ars_to_usdt(
        accountNumber,
        parseInt(depositAmount.split(",").join("").split("$").join("")),
        Number(usdtRate)
      ).then(
        (response) => {
  ...
  const messageRate = () => {
    if (usdtRate && usdtRate > 0) {
      return (
        <div>
          <small>Vas a recibir ${usdtRate} USDT</small>
        </div>
      );
    } else {
      return (
        <div>
          <small>Ingresa un monto</small>
        </div>
      );
    }
  };
